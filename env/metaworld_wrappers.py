...
def make_pad_env(
    ...
    # breakpoint()
    env = mt1.train_classes[task_name](texture=texture)
    env = MetaWorldWrapper(env, mt1, seed, action_repeat)
    # env = FrameStack(env, frame_stack)
    env = MovingAverageWrapper(env, texture, moving_average_denoise=moving_average_denoise, moving_average_denoise_factor=moving_average_denoise_factor, moving_average_denoise_alpha=moving_average_denoise_alpha, k=frame_stack)
...
class MetaWorldWrapper(gym.Wrapper):
    ...
    def __init__(self, env, mt1, seed, action_repeat, action_momentum=0.6, smooth=False):
        ...
        self.success = gym.spaces.Box(0, 1, (), dtype=np.bool)
    ...
    def _get_obs(self):
        ...
        obs = self.env.viewer.read_pixels(width=w, height=h, depth=False)
