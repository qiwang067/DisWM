...
class RSSM(nn.Module):
  ...
  def __init__(
      ...
      cell='gru',
      num_actions=None, embed = None, device=None, rnn_layers=1):
    ...
    if self._shared:
      inp_dim += self._embed
    hidden_inp_dim = inp_dim * 2
    for i in range(self._layers_input):
      inp_layers.append(nn.Linear(inp_dim, self._hidden, bias=True))
  ...
  def fill_zero_action(self, batch_size):
    action = torch.zeros([batch_size, self.num_action]).to(self._device)
    return action
  ...
  def observe_action_free(self, embed, state=None):
    swap = lambda x: x.permute([1, 0] + list(range(2, len(x.shape))))
    if embed.shape[1] == 0:
      b, l, d = embed.shape
    if state is None:
    ...
    embed = swap(embed)
    post, prior = tools.static_scan(
        lambda prev_state, embed: self.obs_step_af(prev_state[0], embed),
      (embed,), (state, state))
    post = {k: swap(v) for k, v in post.items()}
    prior = {k: swap(v) for k, v in prior.items()}
    return post, prior
  ...
  def obs_step_af(self, prev_state, embed):
    prior = self.img_step_af(prev_state, None)
    if self._shared:
      post = self.img_step_af(prev_state, embed)
    else:
      if self._temp_post:
        x = torch.cat([prior['deter'], embed], -1)
      else:
  ...
  def img_step_af(self, prev_state, embed=None):
    ...
    if self._shared:
      if embed is None:
        shape = list(prev_state.shape[:-1]) + [self._embed]
        embed = torch.zeros(shape)
      x = torch.cat([prev_stoch, self.fill_zero_action(prev_stoch.shape[0]), embed], -1)
    else:
      x = torch.cat([prev_stoch, self.fill_zero_action(prev_stoch.shape[0])], -1)
    x = self._inp_layers(x)
    for _ in range(self._rec_depth): # rec depth is not correctly implemented
      deter = prev_state['deter']
      x, deter = self._cell(x, [deter])
