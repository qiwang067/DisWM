...
class Logger:
  ...
  def write(self, fps=False):
    ...
    for name, value in scalars:
      ...
      wandb.log({f'scalars/{name}': value, f'scalars/custom_step': self.step})
    for name, value in self._images.items():
      self._writer.add_image(name, value, self.step)
      wandb.log({f'images/{name}': [wandb.Image(value)], f'scalars/custom_step': self.step})
...
# action_free simulate
def simulate_af(agent, envs, steps=0, episodes=0, state=None):
  # Initialize or unpack simulation state.
  if state is None:
    step, episode = 0, 0
    done = np.ones(len(envs), np.bool_)
